...
POST /draft_forms/:id/publish => publish the forma

Note for frontend => Get :id will give you a json of a forms information, it is the frontends job to maintain it on the client side and send the complete json again start to finish in the PUT request when saving/when the client is closing the window
...
RESPONSES
PATCH /response/:id => edit the current response
